// 数据
        ...
        async function loadData() {
            try {
                ...
                if (!covenantResponse.ok || !operatorResponse.ok) {
        ...
        // 显示本地文件上传提示（针对本地双击打开的情况）
        function showLocalFilePrompt() {
            const uploadPrompt = document.createElement('div');
            uploadPrompt.style.cssText = 'position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: white; padding: 30px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.2); text-align: center; z-index: 2000; width: 80%; max-width: 500px; border: 1px solid #4a9eff;';
            uploadPrompt.innerHTML = `
                <h3 style="color: #ff6b35; margin-bottom: 15px; font-size: 20px;">本地数据加载加载失败</h3>
                <p style="margin-bottom: 15px; color: #555; line-height: 1.5;">受限于浏览器的安全策略，网页直接双击打开时无法自动读取本地的JSON文件。<br>请您点击下方按钮，同时选中目录下的 <b>data_盟约.json</b> 和 <b>data_干员.json</b> 来加载数据。</p>
                <input type="file" id="fallback-file-input" multiple accept=".json" style="display: block; margin: 0 auto 15px auto;">
                <p id="fallback-status" style="color: #4a9eff; font-size: 14px;"></p>
            `;
            document.body.appendChild(uploadPrompt);
            
            document.getElementById('fallback-file-input').addEventListener('change', async (e) => {
                ...
                for (let file of files) {
                    ...
                    reader.onload = (event) => {
                        try {
                            ...
                            if (json && Object.values(json)[0] && Object.values(json)[0]['特质']) {
                                operatorData = json;
                                statusEl.innerHTML += '干员数据加载成功！<br>';
                            } else {
                                covenantData = json;
                                statusEl.innerHTML += '盟约数据加载成功！<br>';
                            }
                            ...
                            statusEl.innerHTML += '<span style="color: red;">解析文件失败: ' + file.name + '</span><br>';
                        }
                    };
